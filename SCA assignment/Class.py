...
class Run:
    ...
    def scenariocreation(self,scenlen,nofsets,stratreturn=0,usestratsol=0,lnko_in=None,lnkcap_in=None):
        ...
        for l in range(nofsets):
            ...
            #print(scen.sum('scenario').values/scenlen) #Check whether across the scenarios the availability rate converges to the average
            ...
            #print(realdem.mean('scenario'))
            #print(realdem.std('scenario'))
            obj,lnko_out,lnkcap_out=self.generalmodel(usestratsol,lnko_in,lnkcap_in)
            #res_list.append(round(obj,2))
    ...
    def generalmodel(self,usestratsol,lnko_in,lnkcap_in):
        ...
        for sc in self.scenario:    
            for i in self.depart:
                for j in self.arrive:
                    for p in self.product:
                        for t in self.time:
                            MILP.addConstr(shipped[sc,i,j,p,t]<=self.M*linkopen[i,j])
        ...
        for sc in self.scenario:
            for p in self.product:
                for d in self.depot:
                    for t in self.time:
                        MILP.addConstr(gb.quicksum(shipped[sc,d,j,p,t] for j in self.arrive)<= inv[sc,p,d,t])

        for i in self.depart:
            for j in self.arrive:
                MILP.addConstr(linkcap[i,j]<=self.M*linkopen[i,j]) # Link capacity can only be procured if the link is first opened
                for t in self.time:
                    for sc in self.scenario:
                        MILP.addConstr(trucksdisp[sc,i,j,t]<=linkcap[i,j]) # Amount of trucks contrained at the procured link capacity per time period

        # Volume of product in depots may not exceed depot size capacity
        for sc in self.scenario:    
            for d in self.depot:
                for t in self.time:
                    MILP.addConstr(gb.quicksum(inv[sc,p,d,t]*self.a.loc[p] for p in self.product)<=self.delta.loc[d])
        ...
        for sc in self.scenario:    
            for i in self.depart:
                for j in self.arrive:
                    for t in self.time:
                        MILP.addConstr(gb.quicksum(shipped[sc,i,j,p,t]*self.a.loc[p] for p in self.product)<=self.gamma*trucksdisp[sc,i,j,t])

        for sc in self.scenario:                
            for s in self.supplier:
                for p in self.product:
                    for t in self.time:
                        MILP.addConstr(self.disrupt.loc[sc,s,p,t].values*production[sc,s,p,t]*self.beta.loc[s,p].values>=gb.quicksum(shipped[sc,s,j,p,t] for j in self.arrive))
...
inst=Run(path="D:/Tilburg University/MSc Programma's/Python/Supply Chain Analytics/uncertainty_data_set.xlsx")

# Vector of SAA solutions of length 'nofsets' where each scenario set contains 'scenlen' number of scenarios
res_list=inst.scenariocreation(scenlen=100,nofsets=1)
...
#objvalSAA,lnko_out,lnkcap_out=inst.scenariocreation(scenlen=30,nofsets=1,stratreturn=1)
...
#lst=inst.scenariocreation(scenlen=1,nofsets=30,usestratsol=1,lnko_in=lnko_out,lnkcap_in=lnkcap_out)
...
G.add_edges_from([tuple((i,j,{'weight':int(lnko_out.loc[i,j])})) for i,j in it.product(inst.depart,inst.arrive) if lnko_out.loc[i,j]>0])
