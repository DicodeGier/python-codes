...
# please change if necessary the directory to the file "data_set.xlsx"
# os.chdir("")
...
[MILP.addConstr(production[s,p,t]*alpha.loc[s,p]<=gb.quicksum(shipped[s,j,p,t] for j in arrive)) for s,p,t in it.product(supplier,product,time)]
...
results.to_excel('OutputTask1.xlsx',index=False)
...
G.add_edges_from([tuple((i,j,{'weight':linkcap[i,j].X})) for i,j in it.product(depart,arrive) if linkopen[i,j].X>0])
