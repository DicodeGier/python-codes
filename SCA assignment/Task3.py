...
# Importing the data:
path="uncertainty_data_set.xlsx" 
...
for i in range(len(DEMAND)):
    row=DEMAND.loc[i,:]
    demand.loc[row['Customer'],row['Product'],row['Time']]=row['Expected Amount']
...
#Previously integer, now continuous allowed to be able to make comparisons with Task 2
trucksdisp = MILP.addVars(depart, arrive, time, vtype=gb.GRB.CONTINUOUS, lb=0, name='trucksdisp')
linkcap = MILP.addVars(depart, arrive, vtype=gb.GRB.CONTINUOUS, lb=0, name='linkcap')
...
# Costs of backlogs
cost4=gb.LinExpr()
for c,p in it.product(customer,product):
    for t in time:
        if t != H:
...
# Constraints
for p,d in it.product(product,depot):
    for t in time_ext:
        if t=='TIMEZERO':
            MILP.addConstr(inv[p,d,t]==0)
        else:
            summed=gb.LinExpr() #Sum of amount of good p arriving to depot d from departure nodes
            for i in depart:
...
[MILP.addConstr(production[s,p,t]*beta.loc[s,p]>=gb.quicksum(shipped[s,j,p,t] for j in arrive)) for s,p,t in it.product(supplier,product,time)]

#Transport only possible if link is opened
[MILP.addConstr(shipped[i,j,p,t]<=M*linkopen[i,j]) for i,j,p,t in it.product(depart,arrive,product,time)]

# Link capacity can only be procured if the link is first opened
[MILP.addConstr(linkcap[i,j]<=M*linkopen[i,j]) for i,j in it.product(depart,arrive)]

# Amount of trucks contrained at the procured link capacity per time period
[MILP.addConstr(trucksdisp[i,j,t]<=linkcap[i,j]) for i,j,t in it.product(depart,arrive,time)]

# Volume of product in depots may not exceed depot size capacity
[MILP.addConstr(gb.quicksum(inv[p,d,t]*a.loc[p] for p in product)<=delta.loc[d]) for d,t in it.product(depot,time)]

# Volume of products in trucks may not exceed truck capacity
[MILP.addConstr(gb.quicksum(shipped[i,j,p,t]*a.loc[p] for p in product)<=gamma*trucksdisp[i,j,t]) for i,j,t in it.product(depart,arrive,time)]

for c,p in it.product(customer,product):
    for t in time:
        cumdif=0 # Cumulative difference in deliveries and demand (mismatch)
...
for s,p in it.product(supplier,product):
    if any([production[s,p,t].X for t in time]):
...
for d,p in it.product(depot,product):
...
for c,p in it.product(customer,product):
    if any([early[c,p,t].X for t in time]):
    ...
    if any([late[c,p,t].X for t in time]):
...
results.to_excel('OutputTask3.xlsx',index=False)
...
G.add_edges_from([tuple((i,j,{'weight':round(linkcap[i,j].X,2)})) for i,j in it.product(depart,arrive) if linkopen[i,j].X>0])
